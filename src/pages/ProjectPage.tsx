  ...
  return (
    <MainLayout>
      <div className="mx-auto max-w-7xl px-5 py-16 lg:px-8 lg:py-20">
        <Link
          to="/"
          className="inline-flex items-center gap-2 text-sm text-[#9cdab1] transition hover:text-[#effff4]"
        >
          <FiArrowLeft />
          Voltar para o portfolio
        </Link>
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[0.7fr_1.3fr]">
          ...
          <div className="glass-card">
            ...
            <p className="mt-5 text-base leading-8 text-[#9fd7b0]">
              {project.description}
            </p>
...
function CaseCard({ title, content }: { title: string; content: string[] }) {
  return (
    <article className="glass-card">
      <h2 className="text-2xl font-semibold text-[#effff4]">{title}</h2>
      <ul className="mt-4 list-disc space-y-3 pl-5 text-base leading-8 text-[#9fd7b0] marker:text-[#4cff8b]">
        {content.map((item) => (
          <li key={item}>{item}</li>
        ))}
      </ul>
